...
require("competitest").setup({
  ...
  received_problems_path = function(task, _)
    if received_position == nil or vim.fn.isdirectory(received_position) == 0 then
      vim.notify("received_position is invalid: " .. tostring(received_position), vim.log.levels.ERROR)
      return
    end
    local domain = task.url:match("^https?://([^/]+)")
    if domain == "open.kattis.com" then
      local id = task.url:match("/problems/([^/]+)")
      return vim.fs.joinpath(received_position, id, id .. ".cpp")
    else
